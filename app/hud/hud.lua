...
function Hud:title()
  ...
  local subtext = 'The ' .. ovw.house.biome
  ...
  elseif ovw.house.currentFloor < 0 then
    subtext = subtext .. ', Basement ' .. math.abs(ovw.house.currentFloor)
  elseif ovw.house.currentFloor == 0 then
    subtext = subtext .. ', Ground Floor'
  else
    subtext = subtext .. ', Floor ' .. ovw.house.currentFloor
  end
...
function Hud:npc()
  ...
  --Draw Crafting Station UI
  elseif npc.npcType == 'CraftingStation' then
    --button
    local button = npc.craftButton
    local alpha = ovw.player.npc.timers.fadeOut * ((npc.items[1] and npc.items[2]) and 155 or 20)
    g.setColor(150, 75, 0, alpha)
    g.rectangle('fill', button.x, button.y, button.width, button.height)
    g.setColor(255, 255, 255, alpha)
    g.rectangle('line', button.x, button.y, button.width, button.height)
    g.printf('Transmute', button.x, button.y + 5, button.width, 'center')

    --slots
    for i = 1, 2 do
      local item = npc.items[i]
      local note = npc.items[i] and (npc.items[i].level and npc.items[i].level or npc.items[i].stacks) or ''
      alpha = ovw.player.npc.timers.fadeOut * (not item and 20 or (npc.items[i].focus and 255 or 200))
      g.setColor(255, 255, 255, alpha)
      if item then g.draw(item.image, 520, 100 + (size + 2) * (i - 1)) end
      g.rectangle('line', 520 + .5, 100 + (size + 2) * (i - 1) + .5, size, size)
      if item then
...
function Hud:flashlight()
  ...
  local ww, hh = 120, 3
  g.hotkeyTab(alpha, 'f', x + ww / 2, y - 2, ww / 2, 10, 'top', nil, nil)
...
function Hud:stamina()
  local p = ovw.player
  local stamina = p:getStat('stamina', true)
  local x = 400 - 50 + .5
  local y = 600 - 90 + .5
  local ww = 100
  local hh = 7
  local val = p.energy / stamina
  local maxVal = stamina
  ...
  g.mouseTab(alpha, 'r', x + ww / 2, y - 2, ww / 2, 10, 'top', nil, {'tab'})
  g.hotkeyTab(alpha, 'e', x + ww, y - 2 + 13, ww / 2, 10, 'top')
  g.setColor(255, math.min(255, 255 * val * 2), math.max(0, 510 * (val - 0.5)), alpha)
  g.drawBar(x, y, ww, hh, val, maxVal)
...
function Hud:experience()
  ...
  labels[1] = ovw.player.agilityModifier > 0 and labels[1] .. ' + ' .. ovw.player.agilityModifier or labels[1]
  labels[2] = ovw.player.armorModifier > 0 and labels[2] .. ' + ' .. ovw.player.armorModifier or labels[2]
  labels[3] = ovw.player.staminaModifier > 0 and labels[3] .. ' + ' .. ovw.player.staminaModifier or labels[3]
...
function Hud:hotbar()
  ...
  for i = 1, 5 do
    ...
    g.hotkeyTab(alpha, i, 2 + (size + 2) * (i - 1) + .5, 2 + .5 + size + 2, size, 10, 'bottom', nil, {'tab'})
    g.setColor(255, 255, 255, alpha)
    ...
    if weapon then
      ...
      if val < 1 then --can reload
        if weapon.selected then
          g.hotkeyTab(alpha, 'r', 2 + .5 + size + 2, 2 + (size + 2) * (i + 1) + .5, 10, size, 'right', nil, {'tab'})
        else
...
function Hud:firstaid()
  ...
  for i = 1, 4 do
    ...
    if bodyPart.crippled then alpha = math.max(alpha, 5) end
    if bodyPart.crippled then alpha = math.max(alpha, 30) end
...
function Hud:mouse()
  ...
  elseif love.keyboard.isDownPausing('e') and ovw.player.npc then
    ...
    --highlight crafting station slots and craft button
    elseif npc.npcType == 'CraftingStation' then

      --button
      local button = npc.craftButton
      if love.mouse.inBox(npc.craftButton.x * love.graphics.getWidth() / w, npc.craftButton.y * love.graphics.getHeight() / h, npc.craftButton.width * love.graphics.getWidth() / w, npc.craftButton.height  * love.graphics.getHeight() / h) then
        g.setColor(255, 255, 0)
        self.mouseText = 'Transmute'
        g.rectangle('line', button.x, button.y, button.width, button.height)
        ovw.player.mouseOverUI = true
      end
  ...
  g.setColor(255, 255, 255, 255)
  local textWidth = love.graphics.getFont():getWidth(self.mouseText or '')
  local textX = love.mouse.scaleX() + 15
  local textX2 = textWidth + textX - w
  if textX2 > 0 then textX = textX - textX2 end
  g.print(self.mouseText or '', textX, love.mouse.scaleY())
end
...
function Hud:mousepressed(x, y, button)
          ...
          if item then
            if self.grabbed.item then
              ...
              elseif button == 'r' and item.stacks then
                --grab one
                self.grabbed.item = _G[string.gsub(item.name, "%s+", "")]()
          ...
          if weapon then
            if self.grabbed.item then
              if self.grabbed.item.type ~= 'Weapon' then
                ...
                if self.grabbed.item.selected then ovw.player.arsenal:select(self.grabbed.item.index) elseif ovw.player.arsenal.weapons[i].selected then ovw.player.arsenal:select(ovw.player.arsenal.weapons[i].index) end
                self:returnGrabbedItem()
              end
            else
              --grab
            ...
            if self.grabbed.item then
              if self.grabbed.item.type ~= 'Weapon' then
                ...
                if ovw.player.arsenal.weapons[i].selected then ovw.player.arsenal:select(i) end
              end
            else
              --you just clicked nothing on an empty slot
            end
  ...
  elseif love.keyboard.isDownPausing('e') then
    ...
    if button == 'l' or button == 'r' then
      ...
      if npc then
              ...
              if item then
                if self.grabbed.item then
                  if self.grabbed.slotType == 'crafting' and (item.type ~= 'Mod' and item.type ~= 'Base') then
                    --can't put not-a-craftable in crafting station!
                  elseif button == 'l' then
                    if self.grabbed.item.name == item.name and item.stacks then
                      --add stack
                      item.stacks = item.stacks + self.grabbed.item.stacks
        ...
        --Interact with Crafting Station
        elseif npc.npcType == 'CraftingStation' then
              ...
              if item then
                if self.grabbed.item then
                  ...
                  elseif button == 'l' then
                    if self.grabbed.item.name == item.name and item.stacks then
                      ...
                      --swap
                      self.grabbed.item, ovw.player.npc.items[i] = ovw.player.npc.items[i], self.grabbed.item
                      ovw.player.npc.items[i].index, self.grabbed.item.index = self.grabbed.item.index, ovw.player.npc.items[i].index
                      self:returnGrabbedItem()
                    end
                  elseif button == 'r' and self.grabbed.item.name == item.name and item.stacks then
                    --pull one from stack
                    item.stacks, self.grabbed.item.stacks = item.stacks - 1, self.grabbed.item.stacks + 1
                    if item.stacks == 0 then ovw.player.npc.items[i] = nil end
                  end
                else
                  if button == 'l' then
                    --grab stack
                    self.grabbed.item, ovw.player.npc.items[i] = ovw.player.npc.items[i], nil
                    self.grabbed.index = self.grabbed.item.index
                    self.grabbed.slotType = 'crafting'
                  elseif button == 'r' and item.stacks then
                    --grab one
                    self.grabbed.item = _G[string.gsub(item.name, "%s+", "")]()
                    self.grabbed.item.stacks = 1
                ...
                if self.grabbed.item then
                  if self.grabbed.item.type ~= 'Mod' and self.grabbed.item.type ~= 'Base' then
                    ...
                    --place
                    ovw.player.npc.items[i], self.grabbed.item = self.grabbed.item, nil
                    ovw.player.npc.items[i].index = i
                  end
                else
                  --you just clicked nothing on an empty slot
                end
  ...
  elseif ovw.player.safety then
    ...
    for i = 1, 5 do
      ...
      if self:mouseOverSlot(2 + (size + 2) * (i - 1) + .5, 2 + .5, size) then
        ...
        if item then
          if button == 'l' then
            --activate item
            ovw.player.hotbar:activate(i, love.keyboard.isDown('lshift'))
          end
        end
        return
      end
    ...
    --change active weapon by clicking on them
    if button == 'l' then
      for i = 1, 2 do
        local weapon = ovw.player.arsenal.weapons[i]
        if self:mouseOverSlot(2 + .5, 2 + (size + 2) * (i + 1) + .5, size) then
...
--if self.grabbed.item then --??? wtf is this for? 'q' maybe?
--  --drop dat bass
--  if button == 'l' then
--  --drop the stack
...
function Hud:returnGrabbedItem()
  ...
  elseif self.grabbed.slotType == 'crafting' then
    if ovw.player.npc then
      if ovw.player.npc.items[index] then
        if ovw.player.npc.items[index].name == item.name then
          --add stacks
          ovw.player.npc.items[index].stacks = ovw.player.npc.items[index].stacks + item.stacks
          self.grabbed.item = nil
          return true
        else
          --you broke it.
        ...
        ovw.player.npc.items[index] = item
        self.grabbed.item = nil
        return true
      end
    else
...
function love.graphics.hotkeyTab(alpha, hkey, tx, ty, tw, th, ts, keyMod, notMods, toggle, toggleSource, toggleVariable)
  ...
  elseif hkey == 'e' and not keyMod then
    pkey = 'Loot'
  else
  ...
  elseif ts == 'left' then
    ...
    --you dumb, keys have to go on one of those sides
  end
end
